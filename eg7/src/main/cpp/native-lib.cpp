...
Java_com_sun_eg7_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
Java_com_sun_eg7_MainActivity_listDirAllFile(JNIEnv *env, jobject instance,
                                             jstring dirPath_) {
    //空判断
    if (dirPath_ == nullptr) {
    ...
    while ((file = readdir(dir)) != nullptr) {
        ...
        if (file->d_type == DT_DIR) {
            ...
            //读取指定文件夹
            Java_com_sun_eg7_MainActivity_listDirAllFile(env, instance, tDir);
            //释放文件夹路径内存
            free(path);
        } else {
            //打印文件名
